...
ggplot(filtered_maize_snp, aes(x = as.numeric(as.character(Position)), fill = Chromosome)) + 
  geom_density() + 
...
ggplot(filtered_teosinte_snp, aes(x = as.numeric(as.character(Position)), fill = Chromosome)) + 
  geom_density() + 
  facet_wrap(~ Chromosome, nrow = 5, ncol = 2) + 
  theme_bw() + 
  labs(x = "Position", y = "Density")
...
ggplot(tidy_fang_genotypes, aes(x = Group , fill = genotype_varieties)) +
              geom_bar(position = "fill") + 
...
ggplot(homozygous_genotypes, aes(x = Position, y = count, color=Genotype)) +
  geom_point() + 
